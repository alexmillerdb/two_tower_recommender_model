...
# MAGIC This notebook illustrates how to create a distributed Two Tower recommendation model. This notebook was tested on g5.24xlarge instances (one instance as the driver, one instance as the worker) using the Databricks Runtime for ML 14.3 LTS. For more insight into the Two Tower recommendation model, you can view the following resources:
# MAGIC
...
# MAGIC **Requirements**
# MAGIC
# MAGIC This notebook requires 14.3 LTS ML.
...
# MAGIC %md ### Helper functions for Converting Pipelineable DataType:
# MAGIC
...
# MAGIC %md #### Helper Function for DataLoading using Mosaic's StreamingDataset
...
# MAGIC %md ### Two Tower Model Architecture
# MAGIC
# MAGIC This section defines the model. For more information, review the original documentation:
# MAGIC - [Hopsworks definition](https://www.hopsworks.ai/dictionary/two-tower-embedding-model)
# MAGIC - [TorchRec's training implementation](https://github.com/pytorch/torchrec/blob/main/examples/retrieval/two_tower_train.py#L75)
...
# MAGIC %md #### Helper functions for Distributed Model Saving
...
# MAGIC %md #### Helper functions for Distributed Model Training and Evaluation
...
if training_method == TrainingMethod.SNMG:
    TorchDistributor(num_processes=num_gpus, local_mode=True, use_gpu=True).run(main, args)
else:
...
if training_method == TrainingMethod.MNMG:
    # args = Args()
    TorchDistributor(num_processes=num_gpus * worker_nodes, local_mode=False, use_gpu=True).run(main, args)
else:
